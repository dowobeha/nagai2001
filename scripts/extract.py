...
class ExtractedGloss:
    ...
    def __init__(self, *, document: str, sentence: int, lines: [str]):
        ...
        self.orthographicWords = list()
        self.phoneticWords = list()
        self.glossedWords = list()
        self.underlyingWords = list()
...
def extractGlosses(input_file: str) -> [Gloss]:
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(f"Usage:\t{sys.argv[0]} in.txt out.tsv")
        sys.exit(-1)
    else:
